...
(function() {
    ...
    findVM().then(vm => {
        ...
        virtualMachine.setVariableValue(virtualMachine.runtime.getSpriteTargetByName("ScratchTools").id,virtualMachine.runtime.getSpriteTargetByName("ScratchTools").lookupVariableByNameAndType("OnMobile").id,7);

        //Mobile check (Thanks to Santhosh on stack overflow: https://stackoverflow.com/a/29509267)

        /*var isMobile = /iPhone|iPad|iPod|Android/i.test(navigator.userAgent);
        if (isMobile) {
            setVar("ScratchTools", "OnMobile", 34)
        } else {
            setVar("ScratchTools", "OnMobile", 34)
        }
        */
